 ...
 * nudges exactly one rabbit other than itself. This other rabbit is chosen with
 * uniform probability. We consider two rabbits to have socialized if either or
 * both of them nudged the other. (Thus many rabbits could have nudged the same
 * rabbit, and two rabbits may have socialized twice.) We consider two rabbits A
 * and B to belong to the same rabbit warren if they have socialized, or if A
 * has socialized with a rabbit belonging to the same warren as B.
 *
 * For example, suppose there were 7 rabbits in Professor Boolean's nefarious
 * lab. We denote each rabbit using a number. The nudges may be as follows:
 *
 ...
 *
 * For example, if there were 4 rabbits, he could infect a maximum of 2 (when
 * they pair up) or 4 (when they're all socialized), but the expected value is
 * 106 / 27. Therefore the answer would be "106/27".
 *
 * Languages =========
 *
 ...
 * Test cases ==========
 *
 * Inputs: (int) n = 4 Output: (string) "106/27"
 *
 * Inputs: (int) n = 2 Output: (string) "2/1"
 *
 */
public class zombit_pandemic {
    ...
    public static String answer(int n) {
        String res = "";
        BigInteger all = BigInteger.valueOf(n - 1).pow(n);
